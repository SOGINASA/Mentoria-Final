...
function People({ employees, stores, search, onSearch, onSelect }) {
  return <div><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h3 className="m-0 font-head text-[20px] font-semibold text-text">Сотрудники</h3><p className="mb-0 mt-1 text-[11px] text-muted">Активный кадровый справочник и связанные аккаунты</p></div><div className="w-full sm:w-80"><PlatformField label="Поиск" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="ФИО, телефон или почта" /></div></div>{employees.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{employees.map((employee) => <button key={employee.id} type="button" onClick={() => onSelect(employee)} className="cursor-pointer rounded-[22px] border border-line bg-surface p-4 text-left shadow-card-sm transition-colors hover:border-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green"><div className="flex items-center gap-3"><span className="grid h-11 w-11 flex-none place-items-center rounded-full bg-brand font-head text-[13px] font-bold text-on-brand">{employee.full_name.split(/\s+/).slice(0, 2).map((part) => part[0]).join('').toUpperCase()}</span><span className="min-w-0 flex-1"><span className="block truncate text-[13px] font-bold text-text">{employee.full_name}</span><span className="mt-1 block truncate text-[10px] text-muted">{[employee.position, stores.find((store) => store.id === employee.store_id)?.name].filter(Boolean).join(' · ') || 'Точка не указана'}</span></span><Icon name="chevronRight" size={18} className="text-faint" /></div><div className="mt-4 flex items-center justify-between gap-3"><StatusPill tone={employee.on_leave ? 'amber' : 'green'}>{employee.on_leave ? 'В отпуске' : 'Активен'}</StatusPill><span className="text-[10px] font-semibold text-muted">{employee.has_account ? `Обучение ${employee.learning.compliance_percent}%` : 'Нет аккаунта'}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-surface2"><div className="h-full rounded-full bg-green" style={{ width: `${employee.learning.compliance_percent}%` }} /></div></button>)}</div> : <EmptyPlatformState icon="users" title="Сотрудники не найдены" subtitle="Измените поисковый запрос или фильтр торговой точки." />}</div>;
}

function EmployeeDetails({ employee }) {
  return <div className="space-y-4"><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl bg-surface2 p-4"><div className="font-head text-[25px] font-semibold text-text">{employee.learning.completed_courses}</div><div className="mt-1 text-[10px] text-muted">курсов завершено</div></div><div className="rounded-2xl bg-surface2 p-4"><div className="font-head text-[25px] font-semibold text-text">{employee.learning.compliance_percent}%</div><div className="mt-1 text-[10px] text-muted">обязательных программ</div></div></div><div className="rounded-2xl border border-line p-4"><div className="text-[11px] font-bold text-text">Контакты</div><div className="mt-3 space-y-2 text-[12px] text-muted"><div>{employee.position || 'Должность не указана'}</div><div>{employee.phone || 'Телефон не указан'}</div><div>{employee.email || 'Почта не указана'}</div><div>{employee.has_account ? 'Аккаунт подключён' : 'Аккаунт приложения не создан'}</div></div></div><ProgressBar value={employee.learning.compliance_percent} label={`${employee.learning.required_completed} из ${employee.learning.required_total} обязательных курсов`} /><div className="flex items-center justify-between rounded-2xl bg-green-tint p-4"><span className="text-[12px] font-semibold text-text">Текущий статус</span><StatusPill tone={employee.on_leave ? 'amber' : 'green'}>{employee.on_leave ? 'Отсутствует' : 'Работает'}</StatusPill></div></div>;
}
