...
const articles = [
  { id: 1, title: 'Обновили стандарт передачи смены', excerpt: 'Теперь ключевые комментарии фиксируются в одном коротком чек-листе.', date: 'Сегодня, 10:20', category: 'Процессы', icon: 'arrowSwap', tone: 'orange' },
  { id: 2, title: 'Новый микроурок по стандартам сервиса', excerpt: 'Четыре минуты, чтобы вспомнить основные шаги общения с гостем.', date: 'Вчера, 16:40', category: 'Обучение', icon: 'book', tone: 'amber' },
  { id: 3, title: 'Команда Достык закрыла месячный план', excerpt: 'Поздравляем коллег и делимся коротким разбором результата.', date: '8 августа', category: 'Команда', icon: 'users', tone: 'green' },
];
...
export default function PlatformNewsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[980px] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-8">
        ...
        {articles.map((article, index) => (
          <button key={article.id} type="button" onClick={() => setSelected(article)} className={`group flex w-full cursor-pointer flex-col gap-4 rounded-[24px] border bg-surface p-5 text-left shadow-card-sm transition-[border-color,background-color,transform] hover:border-green hover:bg-surface2 active:scale-[.995] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green sm:flex-row sm:items-center sm:p-6 ${index === 0 ? 'border-orange' : 'border-line'}`}>
            ...
            <span className="min-w-0 flex-1">
              <span className="flex flex-wrap items-center gap-2"><StatusPill tone={article.tone}>{article.category}</StatusPill><span className="text-[11px] text-faint">{article.date}</span></span>
      ...
      <PlatformModal open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.title} subtitle={selected?.date} footer={<PlatformButton onClick={() => setSelected(null)}>Понятно</PlatformButton>}>
