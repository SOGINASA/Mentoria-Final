...
export default function PlatformReviewerPage() {
  ...
  const [data, setData] = useState({ stores: [], writeOffs: [], queue: EMPTY_QUEUE, analytics: null });
  ...
  const reload = useCallback(async () => {
    ...
    setError('');
    try {
      const params = { status: 'pending', per_page: 50 };
      ...
      setData({
        ...
        writeOffs: writeOffs.write_offs || [], analytics,
      });
    } catch (requestError) {
    ...
    <div className="mx-auto w-full max-w-[1240px] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-8">
      ...
      {error ? <PlatformCard className="mt-6 p-5" variant="orangeTint"><div role="alert" className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-bold text-text">Рабочее пространство не загрузилось</div><div className="mt-1 text-[12px] text-muted">{error}</div></div><PlatformButton variant="secondary" icon="refresh" onClick={reload}>Повторить</PlatformButton></div></PlatformCard> : <>
        <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
          <Metric icon="queue" label="Списания" value={data.writeOffs.length} meta="ожидают решения" tone="orange" />
        ...
        <div className="mt-5 flex flex-col gap-3 rounded-[20px] border border-line bg-surface p-3 shadow-card-sm sm:flex-row sm:items-end">
          ...
          <div className="w-full sm:w-64"><PlatformField as="select" label="Торговая точка" value={storeId} onChange={(event) => setStoreId(event.target.value)}><option value="all">Все закреплённые точки</option>{data.stores.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</PlatformField></div>
        </div>
        ...
        <div key={tab} className="platform-content-swap mt-5">
          {loading ? <div className="grid min-h-64 place-items-center"><Spinner size={30} /></div> : tab === 'queue'
            ? <ReviewQueue writeOffs={data.writeOffs} queue={data.queue} stores={data.stores} onOpenWriteOff={(id) => navigate(`/review/${id}`)} onApprovals={() => navigate(PLATFORM_ROUTES.approvals)} onHistory={() => navigate('/review/history')} />
...
function ReviewQueue({ writeOffs, queue, stores, onOpenWriteOff, onApprovals, onHistory }) {
  const operational = [
